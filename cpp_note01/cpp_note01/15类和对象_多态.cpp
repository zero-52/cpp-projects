...
多态：
	...
	tip：c++中允许父子类型的相互转换，不需要强制类型转换，即父类的引用或指针可以直接指向子类对象
...
纯虚函数 和 抽象类：
 	...
 	因此，可以将 虚函数 改为 纯虚函数，当类中有了纯虚函数，该类也称为 抽象类
 	...
 	抽象类的特点：
 		无法实例化对象（但能创建指针）
 		抽象类的子类必须重写抽象类中的纯虚函数，否则子类也属于抽象类，导致子类也无法实例化对象
 ...
 *  @brief	纯虚函数 和 抽象类
 *		在多态中，通常父类的虚函数实现是无意义的，主要都是调用子类重写的内容
 *		因此，可以将 虚函数 改为 纯虚函数，当类中出现纯虚函数，该类也称为 抽象类
 *
 *	纯虚函数语法：
 *		virtual 返回值类型 函数名(参数列表) = 0;
 *		如 virtual void func(int a, int b) = 0;  //纯虚函数：不做任何事
